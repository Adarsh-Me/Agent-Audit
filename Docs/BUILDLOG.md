...
## Day 20 — Remote MCP: the product is now callable from ChatGPT & Claude (Aug 26)

- **Shipped:** the deployed API serves the MCP streamable-HTTP transport at `https://agentaudit-api.antideploy.com/mcp` — same three tools as `mcp-server/server.mjs` (`audit_status`, `get_report`, `create_payment_link`), backed by the REST router handlers directly so responses match `/api` byte-for-byte, including the SCHEMA §7 error envelope on AppError. Local stdio path unchanged.
- **Transport choices:** stateless (no session ids → no sticky routing across container swaps) + plain-JSON replies (no SSE to buffer). `mcp==1.29.1` pinned — the 2.x line forces starlette>=1.0 which fastapi 0.115.12 forbids.
- **Two SDK-integration traps found and fixed:** (1) FastMCP's session-manager singleton refuses a second `.run()`, which breaks every reload/test cycle → one fresh manager per lifespan entry, mounted as raw ASGI; (2) Starlette's Mount 307s the slash-less exact path, and antideploy's Google frontend **411s the followed POST's body** — hosted clients posting exactly `/mcp` would never connect. Fix: a Request→ASGI→Response bridge route serves `/mcp` in one hop (safe: JSON mode guarantees a single body message).
- **Verified on prod:** `/api/dbstatus` on_primary (Neon); initialize/tools-list/audit_status/get_report over public HTTPS against real suta run `835ef492`; payment tool returns structured policy envelopes. Suite 142 green incl. 8 new `/mcp` contract tests; ruff clean. Deploys backend15/backend16 both succeeded (deploy script's success-string check was wrong twice — platform reports `succeeded`).
- **Connection:** ChatGPT developer-mode connectors / claude.ai custom integrations paste the URL above; Claude Code: `claude mcp add --transport http agentaudit https://agentaudit-api.antideploy.com/mcp`.
- **Ops note:** the old "Mavis Agent" account token was found revoked in the dashboard (explains the vanished `%USERPROFILE%\.antideploy\config.json`); a fresh token came through the documented device flow (`POST /api/v1/device/code` → user approves → poll `device/token` straight into config.json, never echoed).
- **Spent:** $0.
