...
async def test_full_mocked_run_220_done_under_60s(db_env, registry):
    ...
    assert n == 220
    assert dict(tiers) == {"bulk": 200, "flagship": 20}
...
async def test_cost_cap_aborts_partial(db_env, registry):
    ...
    assert n < 220  # aborted mid-matrix; never silently complete
...
async def test_rerun_unchanged_catalog_is_100pct_cached(db_env, registry):
    ...
    assert total == 220 and cached == 220, "unchanged-catalog rerun is ~100% cache-served"
